...
if __name__ == "__main__":
        ...
        for i in range(N_GENERATIONS):
            print(f"\nRunning Generation {i+1}/{N_GENERATIONS}... with a fixed random state of {FIXED_RANDOM_STATE+i}" )
            X , y  = make_dataset1(n_points=N_POINTS , random_state=FIXED_RANDOM_STATE+i)
            ...
            for k in N_NEIGHBORS_VALUES:
                ...
                if i == N_GENERATIONS - 1:  # Only plot for the last generation
                    plot_title = f"k-NN Decision Boundary (k={k}) - Learning Set"
                    filename = f"knn_boundary_k={k}_learningSet"
                    filepath = os.path.join(output_folder, filename)
                    plot_boundary(filepath, knn, X_train, y_train, title=plot_title)
                    print(f"Saved plot to {filepath}.pdf")

                    plot_title = f"k-NN Decision Boundary (k={k}) - Test Set"
                    filename = f"knn_boundary_k={k}_TestSet"
                    filepath = os.path.join(output_folder, filename)
                    plot_boundary(filepath, knn, X_test, y_test, title=plot_title)
                    print(f"Saved plot to {filepath}.pdf")
